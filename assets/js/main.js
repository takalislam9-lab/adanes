...
(function () {
  ...
  async function initConfig() {
    ...
    // Enlace de WhatsApp (número + mensaje predefinido)
    const wa = "https://wa.me/" + (c.whatsapp || "34627112119") +
      "?text=" + encodeURIComponent(c.mensaje_whatsapp || "Hola, quiero reservar una cita.");
    ["#hero-whatsapp", "#footer-whatsapp", "#float-whatsapp", "#form-whatsapp"].forEach((id) => {
      const el = $(id); if (el) el.href = wa;
    });

    // Enlaces de teléfono
    const tel = "tel:" + (c.telefono_link || "+34627112119").replace(/\s+/g, "");
    const hp = $("#hero-phone"); if (hp) hp.setAttribute("href", tel);
  }
    ...
    grid.innerHTML = items
      .map((t, i) => {
        ...
        return (
          ...
          '<img src="' + esc(t.imagen) + '" alt="' + esc(t.titulo) + ' — barbería Adanes Alcobendas" loading="lazy" class="gallery-img w-full h-full object-cover aspect-square" />' +
